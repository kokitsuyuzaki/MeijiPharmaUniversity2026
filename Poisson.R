# =============================================================================
# ポアソン分布（ポアソン回帰）のアニメーション
# 回帰曲線上をポアソン分布がスライドする様子を可視化
# =============================================================================
...
# --- モデルの当てはめ ---
fit <- glm(y ~ x, family = poisson(link = "log"), data = d0)
...
tmpdir <- file.path(tempdir(), "frames_poisson")
...
for (j in seq_along(xrange)) {
    ...
    kmax <- 12
    kvec <- 0:kmax
    pmf <- dpois(kvec, lambda)
    pt_df <- data.frame(x = xj, y = lambda)
    bar_df <- data.frame(
        k = kvec, pmf = pmf,
    ...
    p1 <- ggplot() +
        ...
        coord_cartesian(xlim = c(-1.5, 2.5), ylim = c(-2, 12)) +
        labs(x = "x", y = "y") + theme_anim

    # 右パネル: ポアソン分布のPMF（縦向き棒グラフ）
    p2 <- ggplot(bar_df, aes(x = k, y = pmf)) +
        geom_col(fill = "#F8766D", alpha = 0.8, width = 0.6) +
        coord_cartesian(xlim = c(-1, kmax), ylim = c(0, 1)) +
        labs(x = "y", y = "Probability") +
        annotate("text", x = kmax * 0.75, y = 0.9,
            label = "y ~ Poisson(\u03bb)\nlog(\u03bb) = a + bx",
            size = 4.5) +
        annotate("text", x = kmax * 0.75, y = 0.72,
            label = "\u2193", size = 6) +
        annotate("text", x = kmax * 0.75, y = 0.58,
            label = sprintf(
                "y ~ Poisson(\u03bb)\nlog(\u03bb) = %.3f + %.3f x",
                a, b),
            size = 4.5) +
...
system(paste("convert -delay 10 -loop 0",
    paste(frames, collapse = " "), "Poisson.gif"))

# --- クリーンアップ ---
unlink(tmpdir, recursive = TRUE)
cat("Poisson.gif を生成しました\n")
